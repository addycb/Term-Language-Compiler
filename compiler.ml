...
let rec command_val (x:term) =
  match x with 
    Int(a)-> "Push "^(string_of_int(a))
    |Name(a)-> "Push "^a^" Lookup"
    |Unit->"Push ()" 
    |Add(un1,un2)->
      (command_val(un1))^" "^(command_val(un2))^" Add"
    |Sub(un1,un2)->
      (command_val(un1))^" "^(command_val(un2))^" Sub"
    |Div(un1,un2)->
      (command_val(un1))^" "^(command_val(un2))^" Div"
    |Mul(un1,un2)->
      (command_val(un1))^" "^(command_val(un2))^" Mul"         
    |App(term1,term2)->
      command_val(term1)^" "^(command_val(term2))^" Call"
    |LetIn(name,term1,term2)-> 
     "Begin Push "^name^" "^(command_val(term1)) ^" Let "^(command_val(term2))^" End"  
    |Fun(name1,name2,term)-> 
      "Fun "^name1^" "^name2^" "^(command_val(term))^" End "^"Push "^name1^" Lookup"  
    |Ifgz(term1,term2,term3)->
      (command_val(term1))^" If Begin "^(command_val(term2))^" End Else Begin "^(command_val(term3))^" End End"
    |Trace(term)->(command_val(term))^" Trace"  
    |_->"Error" 
...
let compiler (src : string) : string =
   match (term_parser () (explode src)) with
   Some (x,y)->(command_val x)
   |None->"Error"
